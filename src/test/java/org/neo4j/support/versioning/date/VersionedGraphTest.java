...
public class VersionedGraphTest {
	...
	public void setUp() throws IOException {
        ...
        try (Transaction tx = graphDb.beginTx()) {
            versionNode = graphDb.createNode();
            tx.success();
        }
        versioningTransactionEventHandler = new VersioningTransactionEventHandler(versionNode);
	...
	public void testDiscoveryOfRelationships() {
        ...
        try (Transaction tx = graphDb.beginTx()) {
            assertAdjacency(rel, vc(getLatestVersion()).node(n1), vc(getLatestVersion() - 1).node(n1));
        }
	}
	...
	public void testVersionedProperties() {
		...
		long fooVersion = getLatestVersion();
		setProperty(node, "key", "bar");
		long barVersion = getLatestVersion();
		setProperty(node, "key", "zoo");
		setProperty(node, "other", "asdf");
		long zooVersion = getLatestVersion();
		removeProperty(node, "key");
		long nokeyVersion = getLatestVersion();
	...
	public void testRemovalOfRelationships() {
		...
		long firstVersion = getLatestVersion();
		removeRelationship(firstRel);
		createRelationship(n1, n3, RelTypes.LINKED);
		long secondVersion = getLatestVersion();
	...
	public void testRelationshipMethodsThatReturnVersionedNodes() {
		...
		long snapshot = getLatestVersion();
		createRelationship(n2, n3, RelTypes.LINKED);
	...
	private void assertAdjacency(Relationship rel, VersionedNode overlappingVersionedNode, VersionedNode nonOverlappingVersionedNode) {
		...
		assertEquals(Collections.singleton(rel), addToSet(overlappingVersionedNode.getRelationships()));
		assertEquals(Collections.EMPTY_SET, addToSet(nonOverlappingVersionedNode.getRelationships()));

		assertEquals(Collections.singleton(rel), addToSet(overlappingVersionedNode.getRelationships(Direction.OUTGOING)));
		assertEquals(Collections.EMPTY_SET, addToSet(nonOverlappingVersionedNode.getRelationships(Direction.OUTGOING)));

		assertEquals(Collections.singleton(rel), addToSet(overlappingVersionedNode.getRelationships(RelTypes.LINKED)));
		assertEquals(Collections.EMPTY_SET, addToSet(nonOverlappingVersionedNode.getRelationships(RelTypes.LINKED)));
	}
	...
	private void removeRelationship(Relationship relationship) {
        try (Transaction tx = graphDb.beginTx()) {
			vc(getLatestVersion()).deleteRelationship(relationship);
			tx.success();
		}
	}
	...
	private void removeNode(Node node) {
        try (Transaction tx = graphDb.beginTx()) {
			vc(getLatestVersion()).deleteNode(node);
			tx.success();
		}
	}
    ...
    private long getLatestVersion() {
        try (Transaction tx = graphDb.beginTx()) {
            long version = versioningTransactionEventHandler.getLatestVersion();
            tx.success();
            return version;
        }
    }
